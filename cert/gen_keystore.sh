...
SSH="sshpass -p couchbase ssh -o StrictHostKeyChecking=no"
SCP="sshpass -p couchbase scp -o StrictHostKeyChecking=no"
...
# Add ip address into openssl-san.cnf
cp openssl-org.cnf openssl-san.cnf
...
do
	...
	echo IP.${idx} = ${ip} >> openssl-san.cnf
	idx=$[${idx}+1]
done
...
-CAserial intermediateCA.srl -out ${NODE}.pem -days 365 -extfile openssl-san.cnf -extensions 'v3_req' 2>/dev/null
